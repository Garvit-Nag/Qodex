    ...
    try {
      ...
      if (err.code === 401 || err.message?.includes('Invalid credentials')) {
        if (err.message?.includes('OAuth')) {
          setError('This email is registered with Google. Please use "Continue with Google" instead.');
        } else {
          ...
          <div className="bg-white/90 dark:bg-white/5 backdrop-blur-md border border-gray-300 dark:border-white/20 rounded-2xl shadow-2xl p-6 space-y-5">
            ...
            {forgotPasswordMessage && (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-600 dark:text-green-400 px-3 py-2 rounded-lg backdrop-blur-sm">
                <div className="flex items-center">
                  ...
                  <span className="text-xs font-medium">{forgotPasswordMessage}</span>
                </div>
              </div>
            )}
            ...
            {!showForgotPassword ? (
              ...
              <form className="space-y-3" onSubmit={handleForgotPassword}>
                <div>
                  <label htmlFor="forgot-email" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Email address
                  </label>
                  <input
                    id="forgot-email"
                ...
                <div className="flex space-x-3">
                  <button
                    ...
                    className="flex-1 bg-white/90 dark:bg-white/5 backdrop-blur-md hover:bg-white dark:hover:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white font-medium py-2.5 px-3 rounded-lg transition-all duration-300 text-sm"
                  >
                    Back to Sign In
                  </button>
                  ...
                  <button
                    ...
                    className="flex-1 bg-gray-900 hover:bg-gray-800 dark:bg-gray-200 dark:hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed text-white dark:text-black font-medium py-2.5 px-3 rounded-lg transition-all duration-300 shadow-lg text-sm"
                  >
