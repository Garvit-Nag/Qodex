...
export default function UserProfileSection({ user, userProfile, repositories }: UserProfileSectionProps) {
  // 🔍 Debug logs - INSIDE the component function
        ...
        <div className="relative">
          {user.avatar ? (
            <div className="w-20 h-20 rounded-full overflow-hidden shadow-lg border-2 border-white/20">
              ...
              />
              {/* Fallback initials */}
              <div className="fallback-avatar w-20 h-20 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full flex items-center justify-center text-white text-xl font-bold absolute inset-0">
                {getInitials(user.name)}
              </div>
            </div>
          ) : (
            <div className="w-20 h-20 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-lg">
              {getInitials(user.name)}
            </div>
          )}
        ...
        <div className="bg-white/50 dark:bg-white/10 backdrop-blur-md border border-gray-200 dark:border-white/20 rounded-xl p-4 hover:scale-105 transition-transform duration-300">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Plan Status</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white truncate">
                {isPremium ? 'Code Pro' : 'Code Explorer'}
              </p>
              <p className="text-xs text-gray-500 dark:text-gray-500">
                {isPremium ? 'Unlimited' : '25 messages'}
              </p>
            </div>
          </div>
        </div>
